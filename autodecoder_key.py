...
keys = load('key_words_new.npy')
keys = keys/keys.max()
input_size = keys.shape[1]
code_size = 30
X_train, X_Test = train_test_split(keys, test_size=0.1, random_state=42)
...
history = autoencoder.fit(x=X_train, y=X_train, epochs=80,validation_data=[X_valid, X_valid])
...
encode_topics = encoder.predict(keys)
save('encode_key_new.npy', encode_topics)